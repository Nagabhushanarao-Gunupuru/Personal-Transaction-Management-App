...
import './index.css'
import LastSevenTransactions from "./LastSevenTransactions";
...
class DashBoard extends Component{
    ...
    getTotalDeebitAndCreditData = async()=>{
        ...
        const debitSum = data.totals_credit_debit_transactions.find(item => item.type === "debit")?.sum || 0;
        const creditSum = data.totals_credit_debit_transactions.find(item => item.type === "credit")?.sum || 0;
    ...
    getLastSevenTransactions = async()=>{
        ...
        const lastSevenDebitSum = data.last_7_days_transactions_credit_debit_totals.filter(item => item.type === "debit").reduce((acc, item) => acc + item.sum, 0);
        const lastSevenCreditSum = data.last_7_days_transactions_credit_debit_totals.filter(item => item.type === "credit").reduce((acc, item) => acc + item.sum, 0);

        this.setState({
        lastSevenDebitSum: lastSevenDebitSum,
        lastSevenCreditSum: lastSevenCreditSum,
        lastSevenTransactions:data.last_7_days_transactions_credit_debit_totals
        });
